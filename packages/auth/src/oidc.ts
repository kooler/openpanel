 ...
 *
 * Endpoints are operator-supplied, so they skip `assertSafeUrl`: pointing a
 * deployment at its own internal IdP is the intended use, not SSRF.
 */
...
/**
 * Null when unconfigured. Throws when half-configured so a broken setup
 * never boots.
 */
